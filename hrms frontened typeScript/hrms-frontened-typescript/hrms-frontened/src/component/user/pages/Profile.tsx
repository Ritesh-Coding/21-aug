...
const Profile : React.FC= () => {
  ...
  const { userId,profile } = useSelector((state : RootState) => state.auth);
  ...
  const handleProfilePicSubmit = async (
    ...
    try {
      ...
      const profileImage = response.data.profile.split("/").slice(4).join('/')   

      console.log("profile here",typeof(profileImage),profileImage)
      // dispatch(loginSuccess({profile : "profileImage"}))
